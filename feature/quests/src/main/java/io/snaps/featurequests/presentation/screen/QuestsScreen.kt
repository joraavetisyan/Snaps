...
import io.snaps.featurequests.presentation.historyQuestsItems
...
import io.snaps.featurequests.presentation.viewmodel.QuestsViewModel
...
fun QuestsScreen(
    ...
    val viewModel = hiltViewModel<QuestsViewModel>()
    ...
    viewModel.command.collectAsCommand {
        when (it) {
            QuestsViewModel.Command.ShowBottomDialog -> coroutineScope.launch { sheetState.show() }
            QuestsViewModel.Command.HideBottomDialog -> coroutineScope.launch { sheetState.hide() }
            is QuestsViewModel.Command.OpenQuestDetailsScreen -> router.toQuestDetailsScreen(it.args)
            is QuestsViewModel.Command.OpenNftDetailsScreen -> router.toUserNftDetailsScreen(it.args)
        }
    }
    ...
    ModalBottomSheetLayout(
        ...
        sheetContent = {
            when (uiState.bottomDialog) {
                QuestsViewModel.BottomDialog.CurrentTasksFootnote,
                QuestsViewModel.BottomDialog.HistoryTasksFootnote -> FootnoteBottomDialog(
...
private fun QuestsScreen(
    uiState: QuestsViewModel.UiState,
