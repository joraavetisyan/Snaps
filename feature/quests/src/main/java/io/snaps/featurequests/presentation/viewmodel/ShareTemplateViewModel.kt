...
class ShareTemplateViewModel @Inject constructor(
    ...
    fun onPostToInstagramButtonClicked() {
        viewModelScope.launch {
            ...
            val quest = questsRepository.currentQuestsState.value.dataOrCache ?: return@launch
            if (user?.instagramId == null) {
                notificationsSource.sendError(StringKey.TaskShareErrorNoInstagramConnected.textValue())
                return@launch
            }
            val socialShareTask = quest.quests.find { it.type == QuestType.SocialPost }
            ...
            action.execute {
                questsRepository.postToInstagram()
            }.doOnSuccess {
                notificationsSource.sendMessage(StringKey.TaskShareMessagePostInstagram.textValue())
                onPosted(user, quest)
    ...
    private fun isAllowedToPost(userInfoModel: UserInfoModel, quests: QuestModel): Boolean {
        val date = quests.questDate.toInstant(ZoneOffset.UTC)?.toEpochMilli() ?: return true
        val maxCount = 3
        val currentCount = userDataStorage.getPostedInstagramTemplateCount(userInfoModel.userId, date)
        return (currentCount < maxCount)
    }

    private fun onPosted(userInfoModel: UserInfoModel, quests: QuestModel) {
        val date = quests.questDate.toInstant(ZoneOffset.UTC)?.toEpochMilli() ?: return
        val currentCount = userDataStorage.getPostedInstagramTemplateCount(userInfoModel.userId, date)
