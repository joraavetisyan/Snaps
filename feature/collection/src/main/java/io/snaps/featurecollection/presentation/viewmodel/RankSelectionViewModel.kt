...
class RankSelectionViewModel @Inject constructor(
    ...
    init {
        myCollectionRepository.nftCollectionState.onEach {
        ...
        viewModelScope.launch {
            action.execute { myCollectionRepository.loadNftCollection() }
        }
    }

    private fun subscribeOnRanks(purchasedRanks: List<NftModel>) {
        ranksLoadJob?.cancel()
        ranksLoadJob = myCollectionRepository.ranksState.map {
            it.toRankTileState(
