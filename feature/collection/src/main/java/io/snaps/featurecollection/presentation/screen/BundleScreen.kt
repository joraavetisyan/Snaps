...
import io.snaps.featurecollection.presentation.viewmodel.BundleViewModel
...
fun BundleScreen(
    ...
    val viewModel = hiltViewModel<BundleViewModel>()
    ...
    viewModel.command.collectAsCommand {
        when (it) {
            is BundleViewModel.Command.BackToMyCollectionScreen -> if (it.data != null) {
                router.backToMyCollectionScreenWithResult(it.data)
            } else {
                router.backToMyCollectionScreen()
            }
            is BundleViewModel.Command.CopyText -> clipboardManager.setText(AnnotatedString(it.text))
        }
    }
    ...
    ModalBottomSheetLayout(
        ...
        sheetContent = {
            when (val dialog = uiState.bottomDialog) {
                ...
                is BundleViewModel.BottomDialog.TopUp -> TopUpDialog(
...
@Composable
private fun BundleScreen(
    uiState: BundleViewModel.UiState,
    ...
    Scaffold(
        ...
        Column(
            ...
            uiState.bundle.Content(modifier = Modifier)
            Spacer(modifier = Modifier.height(16.dp))
            uiState.itemsInBundle.forEach {
