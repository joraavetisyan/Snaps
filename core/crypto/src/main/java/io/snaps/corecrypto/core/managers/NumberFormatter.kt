...
import io.snaps.corecrypto.core.IAppNumberFormatter
...
class NumberFormatter(
    ...
    private fun formatRounded(
        ...
        when ((rounded as? BigDecimalRounded.Large)?.name) {
            LargeNumberName.Thousand -> 0
            LargeNumberName.Million -> 0
            LargeNumberName.Billion -> 0
            LargeNumberName.Trillion -> 0
            LargeNumberName.Quadrillion -> 0
            else -> null
        }?.let {
            formattedNumber = Translator.getString(0, formattedNumber, Translator.getString(it))
        }
