...
class Eip1559GasPriceService(
    ...
    private val baseFeeRangeConfig = FeeRangeConfig(
        lowerBound = FeeRangeConfig.Bound.Multiplied(BigDecimal(0.5)),
        upperBound = FeeRangeConfig.Bound.Multiplied(BigDecimal(3.0))
    )
    private val priorityFeeRangeConfig = FeeRangeConfig(
        lowerBound = FeeRangeConfig.Bound.Fixed(0),
        upperBound = FeeRangeConfig.Bound.Multiplied(BigDecimal(10))
    )
    private val overpricingBound = FeeRangeConfig.Bound.Multiplied(BigDecimal(1.5))
    private val riskOfStuckBound = FeeRangeConfig.Bound.Multiplied(BigDecimal(0.9))
    ...
    private fun syncFeeRanges(newRecommendGasPrice: GasPrice.Eip1559) {
        val recommendedBaseFee =
            newRecommendGasPrice.maxFeePerGas - newRecommendGasPrice.maxPriorityFeePerGas
