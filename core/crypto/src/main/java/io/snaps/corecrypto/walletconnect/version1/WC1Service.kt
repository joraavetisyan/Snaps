...
import io.snaps.corecrypto.core.order
...
import io.snaps.corecrypto.walletconnect.other.WCRequestWrapper
...
class WC1Service(
    ...
    val evmKitWrapper: EvmKitWrapper?
        get() = sessionData?.account?.let { account ->
            manager.getEvmKitWrapper(
                evmBlockchainManager.getChain(selectedBlockchain.type).id,
                account
            )
        }
    ...
    fun start() {
        remotePeerId?.let {
            val session =
                sessionManager.sessions.firstOrNull { session -> session.remotePeerId == remotePeerId }
            if (session != null) {
    ...
    private fun restoreSession(session: WalletConnectSession) {
        try {
            ...
            interactor =
                WalletConnectInteractor(session.session, session.peerId, session.remotePeerId)
    ...
    private fun initSession(peerId: String, peerMeta: WCPeerMeta, chainId: Int) {
        selectedBlockchain =
            evmBlockchainManager.getBlockchain(chainId) ?: throw SessionError.UnsupportedChainId
    ...
    override fun didRequestSession(
        remotePeerId: String,
