...
import io.snaps.corecrypto.core.fiat.AmountTypeSwitchService.AmountType
...
class FiatService(
    ...
    private fun fullAmountInfo(): FullAmountInfo? {
        ...
        return when (switchService.amountType) {
            AmountType.Coin -> {
                ...
                FullAmountInfo(
                    primaryInfo = SendModule.AmountInfo.CoinValueInfo(primary),
                    secondaryInfo = secondary?.let {
            ...
            AmountType.Currency -> {
                ...
                FullAmountInfo(
                    primaryInfo = SendModule.AmountInfo.CurrencyValueInfo(primary),
                    secondaryInfo = SendModule.AmountInfo.CoinValueInfo(secondary),
                    coinValue = secondary
                )
            }
        }
    ...
    fun set(token: Token?) {
        ...
        when (switchService.amountType) {
            AmountType.Coin -> fullAmountInfoSubject.onNext(
                Optional.ofNullable(
                    buildForCoin(
                        coinAmount
                    )
            ...
            AmountType.Currency -> fullAmountInfoSubject.onNext(
                Optional.ofNullable(
                    buildForCurrency(
                        currencyAmount
                    )
    ...
    data class FullAmountInfo(
        val primaryInfo: SendModule.AmountInfo,
        val secondaryInfo: SendModule.AmountInfo?,
        val coinValue: CoinValue
    ) {
        val primaryValue: BigDecimal
            get() = when (primaryInfo) {
                is SendModule.AmountInfo.CoinValueInfo -> primaryInfo.coinValue.value
                is SendModule.AmountInfo.CurrencyValueInfo -> primaryInfo.currencyValue.value
            }

        val primaryDecimal: Int
            get() = when (primaryInfo) {
                is SendModule.AmountInfo.CoinValueInfo -> primaryInfo.coinValue.decimal
                is SendModule.AmountInfo.CurrencyValueInfo -> primaryInfo.currencyValue.currency.decimal
            }
    }
