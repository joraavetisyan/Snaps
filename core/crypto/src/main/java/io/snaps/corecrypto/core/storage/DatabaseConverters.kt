...
class DatabaseConverters {
    ...
    fun decryptSecretString(value: String?): SecretString? {
        ...
        return try {
            SecretString(CryptoKit.encryptionManager.decrypt(value))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretString(secretString: SecretString?): String? {
        return secretString?.value?.let { CryptoKit.encryptionManager.encrypt(it) }
    }
    ...
    fun decryptSecretList(value: String?): SecretList? {
        ...
        return try {
            SecretList(CryptoKit.encryptionManager.decrypt(value).split(","))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretList(secretList: SecretList?): String? {
        return secretList?.list?.joinToString(separator = ",")?.let {
            CryptoKit.encryptionManager.encrypt(it)
        }
    }
