...
class EvmFeeService(
    ...
    override var transactionStatus: DataState<Transaction> =
        DataState.Error(GasDataError.NoTransactionData)
        private set(value) {
            field = value
            transactionStatusSubject.onNext(value)
        }
    private val transactionStatusSubject = PublishSubject.create<DataState<Transaction>>()
    override val transactionStatusObservable: Observable<DataState<Transaction>> =
    ...
    private fun sync(gasPriceInfo: GasPriceInfo) {
        ...
        val transactionSingle =
            gasDataService.predefinedGasDataAsync(gasPrice, transactionData)?.map { gasData ->
                Transaction(transactionData, gasData, warnings, errors)

            } ?: if (transactionData.input.isEmpty() && transactionData.value == evmBalance) {
                gasDataService.estimatedGasDataAsync(gasPrice, transactionData, BigInteger.ONE)
                    .map { gasData ->
                ...
                gasDataService.estimatedGasDataAsync(gasPriceInfo.gasPrice, transactionData, null)
                    .map { gasData ->
                        Transaction(transactionData, gasData, warnings, errors)
                    }
            }
